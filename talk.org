...
** Haskell
   ...
   3. It allow us to specify properties, and guarantees within our programs. E.g., this function does not alter global state, or read from a file.
      ...
      + Special actions, like IO, are labeled in the type. So you can't use an IO value in a non-IO function.
        * The IO action would cause the calling function to have an IO type. IO taints values.
   ...
   that problem! However, we can do even better!
...
* Dependent Types in Idris
  ...
  The lower case letters are "variables" again, in this case
  ...
  stand in for a value which Idris will fill in for us.
  ...
  argument, just with one less element since ~x~ is split from it.
...
* Curry-Howard Isomorphism
  ...
  Q. So function application seems to be identical to modus ponens.
  ...
  proposition at the end of the implication chain. Which makes sense
  to me!
...
* Proofs in practice
  ...
  one of the arguments to ~cong~. Since we have a proof that ~x=y~, we
  should be able to rewrite ~y~ to be ~x~ using reflexivity. In Idris
  this is done by deconstructing the proof of ~x=y~ by pattern
  matching on the argument.
...
** Slightly more complicated proof
   ...
   If we give it a function, and something with an equality type, then
   ~cong~ gives us an equality type with the function applied to both
   sides.
...
* Tactics
  ...
  Sometimes building up proof terms in this functional style is a bit
  tedious. Once you get more complicated proofs on the go it gets
  pretty hard to keep track of all of the types. There are other
  languages which use a different style of proof based on tactics,
  which are little commands that build up these proof terms behind the
  scenes for you.
