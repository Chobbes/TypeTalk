...
** Haskell
   ...
   possible valuen, like in the case statement in ~main~. This might
...
* Dependent Types in Idris
  ...
  The lower case letters in the type are "variables" again, in this
  case meaning ~n~ and ~m~ can be any natural numbers, and ~elem~ can
  be any type, this is because ~Vect~ is defined as follows:
...
* Proofs in practice
  ...
  some ~x~ and ~y~ are equal, then ~f x = f y~ as well.
