...
bullet points here, summarizing what I'm talking about.

1) That you should care, and that you NEED to care. Computers are undeniably
   integral to *every* aspect of our lives. Your bank account, car,
   traffic lights, and even the elevator that you might have taken
   today all have computers in them, and they all run software. It
   should terrify you that a loved one might have runs
   software. Possibly very bad software.

   All of these devices have programs on them that need to work
   correctly. Sometimes it's annoying if they don't work correctly,
   which can cause a business to lose customers... And in many cases
   correctness is life or death.
...
* What is a type?
  ...
  programs that make sense! They're excellent documentation, that the
  compiler can ensure is accurate! A type checker can be used to
  reject programs which consist of nonsense like $357^{circles}$, and
  in fact types can eliminate entire classes of errors if designed
  properly.
