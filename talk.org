...
** Haskell
   ...
   you write 3, then it's probably a number. If you multiple a
   variable by another floating point number, then that variable has
   to be a float too. This lets us be as explicit with our types as we
   want, but the compiler can still catch issues even if you don't
   tell it the type of something.
