...
* Proofs in practice
  ...
  the list of known values it seems that we have a proof of ~x=y~ as
  one of the arguments. Since we have a proof that ~x=y~, we should be
  able to rewrite ~y~ to be ~x~ using reflexivity. In Idris this is
  done by deconstructing the proof of ~x=y~ by pattern matching on the
  argument.
  ...
  Perfect! It replaced ~y~ with ~x~ everywhere. Now we just need
  something with the type ~f x = f x~, which is trivial, since if you
  look at the definition of ~Refl~, that's pretty much exactly what it
  does.

  If we give Refl the arguments ~f x~, it should produce a type ~f x =
  f x~. In Idris ~Refl~ uses implicit arguments, since it can often
  infer what it should use in context, so we could just write:
